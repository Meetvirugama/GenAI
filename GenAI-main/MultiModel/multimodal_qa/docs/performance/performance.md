...
- Multi-modal vision analysis (via the `describe_image` BaseTool) natively implements asynchronous execution via `asyncio.to_thread`.
- This ensures that heavy Base64 image encoding and synchronous HTTP blocking calls to the Gemini/Groq APIs do not block the FastAPI event loop during stream token generation.
