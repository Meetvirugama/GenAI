...
def test_main():
    def _run():
        from main import app, init_db, check_dependencies, Config
        check_dependencies()
        init_db()
    safe_run(_run)

def test_agent_methods():
    with patch("app.agent.workflow.ChatGroq") as mock_groq:
...
def test_retrieval_strategies():
    from app.rag.retrieval.strategies import (
        BM25Strategy, DenseStrategy, HybridStrategy, MMRStrategy,
        RRFStrategy, ParentStrategy, CompressionStrategy, MultiQueryStrategy, HyDEStrategy
    )
    mock_vs = MagicMock()
...
def test_vector_store():
    with patch("app.rag.vector_store.PineconeVectorStore") as mock_pinecone, \
