...
def test_all_api_endpoints(mock_redis):
    ...
    # 3. Upload
    try:
        with patch("app.api.endpoints.chat.process_document_task") as mock_task, \
             patch("app.api.endpoints.chat.validate_upload_file") as mock_val:
    ...
    try:
        ...
        with patch("app.api.endpoints.chat.os.path.exists", return_value=True), \
             patch("app.api.endpoints.chat.os.listdir", return_value=["test.png"]):
            resp = client.get("/api/sessions/test/messages")
    except: pass
        
    # 5. Delete Session
    try:
        with patch("app.api.endpoints.chat.shutil.rmtree"):
    ...
    # 7. Tasks
    try:
        with patch("app.api.endpoints.chat.AsyncResult") as mock_ar:
