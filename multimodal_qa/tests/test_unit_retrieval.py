...
class TestBGEReranker:
    ...
    def test_init_without_sentence_transformers(self):
        ...
        with patch.dict("sys.modules", {"sentence_transformers": None}):
            ...
            import app.services.rag.retrieval.reranker as m
            importlib.reload(m)
...
class TestDocumentRetriever:
    ...
    def test_search_no_docs_found_returns_message(self):
        ...
        with patch("app.core.context.session_id_var") as mock_var:
            ...
            with patch("app.services.rag.document_retriever._rerank_docs", return_value=[]):
    ...
    def test_search_returns_xml_format(self):
        ...
        with patch("app.core.context.session_id_var") as mock_var:
            mock_var.get.return_value = "sess"
            with patch("app.services.rag.document_retriever._rerank_docs", return_value=[doc]):
...
class TestRerankDocs:
    def test_returns_subset_when_no_reranker(self):
        from app.services.rag.document_retriever import _rerank_docs
        docs = [Document(page_content=f"doc{i}") for i in range(10)]
        with patch("app.services.rag.document_retriever._get_reranker", return_value=None):
    ...
    def test_reranker_sorts_by_score(self):
        ...
        with patch("app.services.rag.document_retriever._get_reranker", return_value=mock_reranker):
...
class TestGetReranker:
    def test_loads_cross_encoder(self):
        import app.services.rag.document_retriever as mod
        from app.services.rag.document_retriever import _get_reranker
...
class TestRetrievalStrategies:
    def test_dense_strategy_build(self):
        from app.services.rag.retrieval.strategies import DenseStrategy
        mock_vs = MagicMock()
    ...
    def test_mmr_strategy_build(self):
        from app.services.rag.retrieval.strategies import MMRStrategy
        mock_vs = MagicMock()
        mock_vs.as_retriever.return_value = MagicMock(spec=BaseRetriever)
    ...
    def test_bm25_strategy_with_docs(self):
        from app.services.rag.retrieval.strategies import BM25Strategy
        mock_vs = MagicMock()
        mock_vs.get.return_value = {
            "documents": ["doc1 content", "doc2 content"],
            "metadatas": [{"source": "a"}, {"source": "b"}]
        }
        with patch("app.services.rag.retrieval.strategies.BM25Retriever") as mock_bm25:
    ...
    def test_hybrid_strategy(self):
        from app.services.rag.retrieval.strategies import HybridStrategy
        mock_vs = MagicMock()
        mock_vs.as_retriever.return_value = MagicMock(spec=BaseRetriever)
    ...
    def test_rrf_strategy_builds(self):
        from app.services.rag.retrieval.strategies import RRFStrategy
        mock_vs = MagicMock()
        mock_vs.as_retriever.return_value = MagicMock(spec=BaseRetriever)
        mock_vs.get.return_value = {"documents": [], "metadatas": []}
    ...
    def test_rrf_get_relevant_documents(self):
        ...
        with patch("app.services.rag.retrieval.strategies.DenseStrategy") as mock_d, \
             patch("app.services.rag.retrieval.strategies.BM25Strategy") as mock_b:
    ...
    def test_parent_strategy(self):
        from app.services.rag.retrieval.strategies import ParentStrategy
        mock_vs = MagicMock()
        mock_vs.as_retriever.return_value = MagicMock(spec=BaseRetriever)
...
class TestRetrieverFactory:
    def test_factory_returns_retriever_for_dense(self):
        from app.services.rag.retrieval.base import RetrievalStrategy
        from app.services.rag.retrieval.factory import RetrieverFactory
        mock_vs = MagicMock()
        mock_vs.as_retriever.return_value = MagicMock(spec=BaseRetriever)
    ...
    def test_factory_with_reranker(self):
        ...
        mock_vs = MagicMock()
        mock_vs.as_retriever.return_value = MagicMock(spec=BaseRetriever)
        with patch("app.services.rag.retrieval.reranker.BGEReranker") as mock_reranker:
