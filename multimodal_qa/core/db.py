...
class Message(Base):
    ...
    feedback = Column(Integer, nullable=True) # 1 for up, -1 for down, None for no feedback
...
def init_db():
    try:
        ...
        # Safely add feedback column if it doesn't exist
        with engine.begin() as conn:
            try:
                ...
                pass
            try:
                conn.execute(text("ALTER TABLE messages ADD COLUMN trace TEXT"))
            except Exception as e:
                pass
