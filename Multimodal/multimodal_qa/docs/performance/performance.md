...
Both PDF extraction (PyMuPDF) and Vector Embedding (HuggingFace/Pinecone) are CPU-bound, synchronous blocking operations.
If they were run directly in the Gradio event handlers, the entire web server would freeze for all users until the PDF finished processing.

**Solution:** The application utilizes `await asyncio.to_thread(func)` for all document uploading and clearing operations in `ui/app.py`. This offloads the heavy processing to background worker threads, allowing the Gradio async event loop to continue serving other concurrent users immediately.
...
- The Gradio `build_app()` function receives pre-instantiated dependencies (`vector_store`, `agent`).
