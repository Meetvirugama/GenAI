...
**Multimodal Q&A Pro** is an advanced hybrid artificial intelligence agent. It is designed to intelligently process and answer complex questions across multiple formats of data. It integrates local document analysis, image comprehension, and live internet research into a single conversational interface.
...
- **ui:** Contains the legacy Gradio implementation used as a fallback.
...
2. Install the Python dependencies using the package manager.
3. Copy the environment template and configure your secrets in the `.env` file.
4. Add your Groq API Key.
5. Set up your Google OAuth credentials.
6. Ensure you set a strong secret key for session management.
7. Start the FastAPI backend server using Python.
8. The backend will now be running locally on port 7860.

### Frontend Setup Instructions
1. Open a new terminal window and navigate to the frontend directory.
2. Install the Node dependencies using the package manager.
3. Start the Vite development server.
4. The React application will now be running locally on port 5173.
